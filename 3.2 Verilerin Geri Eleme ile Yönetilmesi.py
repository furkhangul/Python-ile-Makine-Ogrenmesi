...
#P değerini hesaplayıp geriye eleme yöntemini uygulayacak olursak:

#Kütüphane indirmek için:
import statsmodels.api as sm

x = np.append(arr=np.ones((14,1)).astype(int), values=toplam.iloc[:,:-1], axis=1)
x_l = toplam.iloc[:,[0,1,2,3,4,5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())

#4. sütun en yüksek olduğu için 4. sütunu çıkarıp tekrar deniyoruz.

x_l = toplam.iloc[:,[0,1,2,4,5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())

#3.sütunu çıkarıyoruz.
x_l = toplam.iloc[:,[0,1,4,5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())

#1.sütunu çıkarıyoruz
x_l = toplam.iloc[:,[1,4,5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())

#2.sütunu çıkarıyoruz
x_l = toplam.iloc[:,[1,5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())

#1.sütunu .ıkarıyorum
x_l = toplam.iloc[:,[5]].values
x_l = np.array(x_l,dtype=float)
model =  sm.OLS(toplam.iloc[:,-1:],x_l).fit()
print(model.summary())
